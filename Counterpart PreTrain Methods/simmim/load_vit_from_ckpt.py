"""
Extracting backbone from a specified SimMIM checkpoint.

Example:

python load_vit_from_ckpt.py \
    --checkpoint ./output/simmim_pretrain/vit_run/ckpt_epoch_99.pth \
    --save-to ./output/models/ \
    --save-name vit_simmim_16_224.pth \
    --num-classes 2
"""
...
def main(args):
    ...
    for seq, src_k in enumerate(ckp_state_dict.keys()):
        if "encoder." in src_k:
            tgt_k = str(src_k).replace("encoder.", "")
...
def get_args_parser():
    ...
    parser.add_argument('--save-name', default='vit_simmim_16_224.pth', type=str, required=True,
