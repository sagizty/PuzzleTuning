...
python -m torch.distributed.launch \
    ...
    main_simmim.py \
        ...
        --data-path /data/imagenet_ILSVRC/ILSVRC/Data/CLS-LOC/train \
        --batch-size 128 \
        --output ./output \
        --tag test_run \
        --amp-opt-level O0
...
def parse_option():
    ...
    # others
    parser.add_argument('--load-weight', type=str, help='Path to init model weight (only applicable for vit model)')
...
def main(config, args):
    ...
    model = build_model(config, is_pretrain=True, load_weight=args.load_weight)
    model.cuda()
    logger.info(str(model))

    optimizer = build_optimizer(config, model, logger, is_pretrain=True)

    # Modified: use scalar for AMP
    assert config.AMP_OPT_LEVEL == "O1", "Only support amp opt level: O1!"
...
def train_one_epoch(config, model, data_loader, optimizer, epoch, lr_scheduler, scaler, writer):
    ...
    for idx, (img, mask, _) in enumerate(data_loader):
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            writer.add_scalar('loss', loss_meter.val, global_step=epoch*len(data_loader)+idx)
    
    epoch_time = time.time() - start
    logger.info(f"EPOCH {epoch} training takes {datetime.timedelta(seconds=int(epoch_time))}")

    # writer.add_scalar('loss', loss_meter.val, global_step=epoch)
