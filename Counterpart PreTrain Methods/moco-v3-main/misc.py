"""
funcs     Script  ver： Aug 15th 19:00
有修改loss backward
"""
import builtins
import datetime
...
def init_distributed_mode(args):
    """
    配置多服务器环境文件信息，安排args.distributed
    :param args:
    :return:
    """
    if args.dist_on_itp:
...
class NativeScalerWithGradNormCount:
    ...
    def __call__(self, loss, optimizer, clip_grad=None, parameters=None, create_graph=False, update_grad=True):
        ...
        # 反传
        # fixme 这里有修改 加了torch.ones_like(loss)解决梯度标量问题，不知道为啥存在
        # fixme 反转了，后来发现又不用加上torch.ones_like(loss)
        self._scaler.scale(loss).backward(create_graph=create_graph)  # create_graph
