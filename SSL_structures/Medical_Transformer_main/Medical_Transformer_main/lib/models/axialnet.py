...
class medt_net(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        x = torch.add(x, x1)
        x = F.relu(F.interpolate(self.decoder5(x) , scale_factor=(2,2), mode ='bilinear'))  # 256*s->128*s->256*s
        # print(x.shape)
        ...
        for i in range(0,4):
            for j in range(0,4):
                ...
                x_p = xin[:,:,32*i:32*(i+1),32*j:32*(j+1)]  # 分patch对每一patch进行transformer，循环操作。
                # begin patch wise
                x_p = self.conv1_p(x_p)  # imgchans -> inplanes 3>512*s
                x_p = self.bn1_p(x_p)
                # x = F.max_pool2d(x,2,2)
                x_p = self.relu(x_p)

                x_p = self.conv2_p(x_p)  # inplanes -> 128
                x_p = self.bn2_p(x_p)
                # x = F.max_pool2d(x,2,2)
                x_p = self.relu(x_p)
                x_p = self.conv3_p(x_p)  # 128->inplanes
                x_p = self.bn3_p(x_p)
                # x = F.max_pool2d(x,2,2)
                x_p = self.relu(x_p)
                ...
                # x = self.maxpool(x)
                # pdb.set_trace()
                x1_p = self.layer1_p(x_p)  # inplanes ->128*s*2, inplanes: 512*s->128*s*2
                # print(x1.shape)
                x2_p = self.layer2_p(x1_p)  # 256*s*2
                # print(x2.shape)
                x3_p = self.layer3_p(x2_p)  # 512*s*2
                # # print(x3.shape)
                x4_p = self.layer4_p(x3_p)  # 1024*s*2
                
                x_p = F.relu(F.interpolate(self.decoder1_p(x4_p), scale_factor=(2,2), mode ='bilinear'))  # 1024*s*2
                x_p = torch.add(x_p, x4_p)
                x_p = F.relu(F.interpolate(self.decoder2_p(x_p) , scale_factor=(2,2), mode ='bilinear'))  # 1024*s
                x_p = torch.add(x_p, x3_p)
                x_p = F.relu(F.interpolate(self.decoder3_p(x_p) , scale_factor=(2,2), mode ='bilinear'))  # 512*s
                x_p = torch.add(x_p, x2_p)
                x_p = F.relu(F.interpolate(self.decoder4_p(x_p) , scale_factor=(2,2), mode ='bilinear'))  # 256*s
